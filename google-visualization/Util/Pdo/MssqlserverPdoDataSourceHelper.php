    ...
    protected static function buildWhereClauseForComparisonFilter(ComparisonFilter $filter)
    {
      ...
      } else // ColumnValueFilter
      {
        ...
        if ($filter->getValue()->getType() == ValueType::TEXT
          || $filter->getValue()->getType() == ValueType::DATE $filter->getValue()->getType() == ValueType::DATETIME $filter->getValue()->getType() == ValueType::TIMEOFDAY)
        {
    ...
    protected static function metaDataToValueType($metaData)
    {
      switch ($metaData["sqlsrv:decl_type"])
      {
        ...
        case "bit":
          ...
          break;
        default:
          throw new TypeMismatchException("MS SQL Server data type '" . $metaData["sqlsrv:decl_type"] . "' cannot be matched to a ValueType");
      }
      return $valueType;
    }
