...
use wpdb;
  ...
  class WPDataSourceHelper
  {
    public static function executeQuery(Query $query, wpdb $db, $tableName)
    {
    	
      mysqli_report(MYSQLI_REPORT_STRICT);
      $queryString = static::buildSqlQuery($query, $tableName);
    // echo 'monkey'.$queryString;
      ...
      try
      {
        ...
        $result = $db->get_results($db->prepare($queryString), ARRAY_N);
       
        if ($result==NULL) {
        	throw new DataSourceException(ReasonType::INTERNAL_ERROR, $db->last_error);
        }
     
        return self::buildTable($db, $result, $columnIdsList);
    ...
    public static function buildTable($db, $result, $columnIdsList = NULL)
    {
      $table = self::buildColumns($db, $columnIdsList);
    ...
    protected static function buildColumns($db, $columnIdsList)
    {
    
      $dataTable = new DataTable();
       $nameCols = $db->get_col_info('name');

	for ($i = 0; $i < count($nameCols); $i++) {
     	
        $id = is_null($columnIdsList) ? $nameCols[$i] : $columnIdsList[$i];
 
      	$valueType = self::fieldToValueType($db->get_col_info('type', $i));
        $columnDescription = new ColumnDescription($id, $valueType, $nameCols[$i] );
        $dataTable->addColumn($columnDescription);
      }
      return $dataTable;
    }

    protected static function buildRows(DataTable $dataTable, $result)
    {
...
( SELECT `t`.*, @row := @row + 1 AS `rownum` FROM (SELECT @row := 0) AS `r`, (" . $queryString. ") AS `t`
    ...
    protected static function getColumnId(AbstractColumn $abstractColumn)
    {
      ...
      } else if ($abstractColumn instanceof AggregationColumn)
      {
        ...
        $columnId = self::getAggregationFunction($abstractColumn->getAggregationType()) . "(cast(" . self::getColumnId($abstractColumn->getAggregatedColumn()) . " as decimal))";
    ...
    protected static function getScalarFunction(ScalarFunctionColumn $col)
    {
	  ...
	  $columns = $col->getColumns();
          return "CAST(" . self::getColumnId($columns[0]). " AS DECIMAL)" ;
          break;
        case "ToDateBoundary":
          $columns = $col->getColumns();
          return "FROM_UNIXTIME(UNIX_TIMESTAMP(" . self::getColumnId($columns[0]). ") - UNIX_TIMESTAMP(" . self::getColumnId($columns[0]). ") mod ". self::getColumnId($columns[1]). ")" ;
        default:
    ...
    protected static function fieldToValueType($type)
    {
      switch ($type)
      {
        case 2:		// SMALLINT
