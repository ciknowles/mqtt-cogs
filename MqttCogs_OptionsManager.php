...
class MqttCogs_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'mqtt-cogs'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'mqtt-cogs'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'mqtt-cogs'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'mqtt-cogs'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'mqtt-cogs');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'mqtt-cogs'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'mqtt-cogs');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'mqtt-cogs'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'mqtt-cogs') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'mqtt-cogs');
            case 'false':
                return __('false', 'mqtt-cogs');

            case 'Administrator':
                return __('Administrator', 'mqtt-cogs');
            case 'Editor':
                return __('Editor', 'mqtt-cogs');
            case 'Author':
                return __('Author', 'mqtt-cogs');
            case 'Contributor':
                return __('Contributor', 'mqtt-cogs');
            case 'Subscriber':
                return __('Subscriber', 'mqtt-cogs');
            case 'Anyone':
                return __('Anyone', 'mqtt-cogs');
        }
        return $optionValue;
    }
